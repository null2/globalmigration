...
exports.process = function(options, done) {
  ...
  csv()
   ...
   .on('end', function() {
      var keys = Object.keys(data.regions).sort().reduce(function(sum, region) {
        sum.indices.push(sum.keys.length);
        sum.keys.push(region);
        sum.keys = sum.keys.concat(data.regions[region].sort());
        return sum;
      }, { indices: [], keys: [] });
