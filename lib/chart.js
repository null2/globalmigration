...
(function(scope) {
  ...
  scope.chart = function(data, config) {
    ...
    function chordInfo(i, d, el) {
      ...
      var text = i.select('.text').selectAll('text')
        .data([
      ...
      i.selectAll('rect').style('fill', d3.select(el).style('fill'));

      var tbbox = i.select('.text').node().getBBox();
      i.select('rect')
        .attr({
    ...
    function groupInfo(i, d, el) {
      ...
      i.selectAll('rect').style('fill', color);
    ...
    function draw(year, countries) {
      ...
      // text on path
      var groupTextPath = groupText.selectAll('textPath')
        .data(function(d) { return [d]; });
      groupTextPath.enter()
        .append("textPath");
      groupTextPath
        .text(function(d) { return data.names[d.id]; })
      groupTextPath.exit().remove();
